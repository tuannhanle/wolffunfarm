...
namespace App.Scripts.UI
{
    public class HeadUpDisplayUI : MiddlewareBehaviour
    {
        ...
        private void Awake()
        {
            ...
            this.Subscribe<ShareData.PlotPassage>(OnDataUpdated);
      }
        ...
        private void OnDataUpdated<T>(T data)
        {
            ...
            else if (typeof(T) == typeof(ShareData.PlotPassage))
            {
                var passager =  data as ShareData.PlotPassage;
                _texts[5].text = $"Unused Plots: {passager.UnusedAmount}";
                _texts[6].text = $"Using Plots: {passager.UsingWorkerAmount}";
            }
